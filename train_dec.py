...
from cgma_net_v1 import msp_decoder as decoder
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    print('Memory Allocated bef:',mem_alloc, 'Maximum memory bef:',  max_mem)
...
def train_model(args):
    ...
    #################################################################################################
    if args.weights:
        model_wt = torch.load(args.weights)
        model_enc.load_state_dict(model_wt['model'])
    else:
        init_weight(model_enc, nn.init.kaiming_normal_, nn.BatchNorm2d, 1e-3, 0.1, mode='fan_in')

    model = decoder(model_enc)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # save the model
        model_file_name = args.savedir + '/model_best_' + str(epoch + 1) + '.pth'
        model_checkpoint = args.savedir + '/checkpoint.pth'
        state = {"epoch": epoch + 1, "model": model.state_dict()}

        if mIOU_val >= max_iou: # >= args.max_epochs - 10:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="CGMANet", help="model name: CGMA Net")
    ...
    parser.add_argument('--lr', type=float, default=4.5e-3, help="initial learning rate")
    parser.add_argument('--batch_size', type=int, default=6, help="the batch size is set to 16 for 2 GPUs")
    parser.add_argument('--savedir', default="./cgma_net/", help="directory to save the model snapshot")
    ...
    parser.add_argument('--weights', type=str, default='./model_292.pth', help="best encoder weights") 
